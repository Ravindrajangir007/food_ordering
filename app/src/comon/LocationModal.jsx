...
const LocationModal = ({ isOpen, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    const fetchLocation = async () => {
      try {
        ...
        const addressData = await getCurrentAddress(location.lat, location.lng);
        if (addressData) {
          setCurrentAddress({ ...location, ...addressData });
        } else {
  ...
  useEffect(() => {
    const handleSearch = async (keyword) => {
      ...
      try {
        const predictions = await searchAddresses(keyword);
